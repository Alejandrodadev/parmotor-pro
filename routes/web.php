...
use App\Http\Controllers\articles\articles;
use App\Http\Controllers\authentications\RegisterController;
use App\Http\Controllers\comparator\comparator;
...
Route::middleware(['auth:sanctum', config('jetstream.auth_session'), 'verified'])->group(function () {
    ...
    // Role: admin|writer for crud motorcycle
    Route::group(['middleware' => ['role:admin|writer']], function () {
        Route::controller(register::class,)->group(function () {
            Route::get('/new-motorcycle', 'index')->name('register.index');
            Route::post('/new-motorcycle', 'newMotorcycle')->name('newMoto');
            Route::get('/motorcycle/{id}', 'edit')->name('motorcycle.edit');
            Route::post('/motorcycle', 'show')->name('motorcycle.show');
            Route::put('/motorcycle/{id}', 'update')->name('motorcycle.update');
            Route::delete('/motorcycle/{id}', 'delete')->name('motorcycle.delete');
        });
    });

    // Role admin only
    Route::group(['middleware' => ['role:admin']], function () {
            Route::get('/laravel/user-management', [UserManagement::class, 'UserManagement'])->name('user-management');
            Route::resource('/user-list', UserManagement::class);

    //  create route for crud of roles and permision controller
        Route::controller(roles::class,)->group(function () {
            Route::resource('/roles', roles::class);
        });
    });

    // Role admin|writer for crud articles
    Route::group(['middleware' => ['role:admin|writer']], function () {
        Route::controller(articles::class,)->group(function () {
            ...
            Route::get('/admin/articles', 'admin')->name('articles.admin');
        });
    });
    ...
    Route::get('/dashboard', $controller_path . '\dashboard\Ecommerce@index')->name('dashboard');
    ...
    //  create route for comparator controller
    Route::controller(comparator::class,)->group(function () {
        Route::get('/comparator', 'index')->name('comparator');
        Route::get('/get-models/{brand}', 'getModelsByBrand')->name('getModelsByBrand');
        Route::post('/show-motorcycles', 'showMotorcycles')->name('showMotorcycles');
    });

    //  create route for articles controller
    Route::controller(articles::class,)->group(function () {
        Route::get('/articles', 'index')->name('articles.index');
        Route::get('/articles/{id}', 'show')->name('content.articles.show');
    });
